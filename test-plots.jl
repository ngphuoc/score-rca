...
# Let r(t) be a vector-valued function with values in Rd, d being 2 or 3. A familiar example is the equation for a line that travels in the direction of →v and goes through the point P: r(t)=P+t⋅→v. A parametric plot over [a,b] is the collection of all points r(t) for a≤t≤b .  In Plots, parameterized curves can be plotted through two interfaces, here illustrated for d=2 : plot(f1, f2, a, b) or plot(xs, ys). The former is convenient for some cases, but typically we will have a function r(t) which is vector-valued, as opposed to a vector of functions. As such, we only discuss the latter.

# An example helps illustrate. Suppose r(t)=⟨sin(t),2cos(t)⟩ and the goal is to plot the full ellipse by plotting over 0≤t≤2π. As with plotting of curves, the goal would be to take many points between a and b and from there generate the x values and y values.
...
# A parametrically described curve in 3D is similarly created. For example, a helix is described mathematically by r(t)=⟨sin(t),cos(t),t⟩ . Here we graph two turns:
...
# Visualizing functions from R2→R
# If a function f:R2→R then a graph of (x,y,f(x,y)) can be represented in 3D. It will form a surface. Such graphs can be most simply made by specifying a set of x values, a set of y values and a function f , as with:
...
# Returning to the The contour plot of f:R2→R draws level curves, f(x,y)=c, for different values of c in the x−y plane. They are produced in a similar manner as the surface plots:

contour(xs, ys, f)

# The cross in the middle corresponds to c=0 , as when x=0 or y=0 then f(x,y)=0 .  Similarly, computed values for f(x,y) can be passed in. Here we change the function:
...
# Let z=f(x,y) describe a surface, and F(x,y,z)=f(x,y)−z. The the gradient of F at a point p on the surface, ∇F(p), will be normal to the surface and for a function, f(p)+∇f⋅(x−p) describes the tangent plane. We can visualize each, as follows:
...
# As illustrated, we can plot surfaces of the form (x,y,f(x,y) . However, not all surfaces are so readily described. For example, if F(x,y,z) is a function from R3→R, then F(x,y,z)=c is a surface of interest. For example, the sphere of radius one is a solution to F(x,y,z)=1 where F(x,y,z)=x2+y2+z2 .
...
# Here we define functions that represent (x,y,z) coordinates in terms of the corresponding spherical coordinates (r,θ,ϕ) .
...
# There is no built in functionality in Plots to create surface described by F(x,y,z)=c . An example of how to provide some such functionality for PyPlot appears here. The following implements the same approach for Plots.
