using Revise
include("rca.jl")
@unpack micro_service_data, get_micro_service_dag, create_observed_latency_data, unobserved_intrinsic_latencies_normal, unobserved_intrinsic_latencies_anomalous = pyimport("micro_service")
...
ground_truth_dag, target_node, ordered_nodes, normal_data, normal_noise = micro_service_data(n_samples)
ordered_nodes = collect(ordered_nodes)
nodes = sub_nodes = @> ordered_nodes Symbol.()
target = Symbol(target_node)
d = length(sub_nodes)
g0, v0, i0_ = from_pydigraph(ground_truth_dag, ordered_nodes)
bn0 = create_model_from_ground_truth_dag(g0, nodes, normal_data)
normal_samples = rand(bn0, n_samples)
learned_dag = fit_dag!(pydeepcopy(ground_truth_dag), pytable(normal_samples))
df_normal_samples = normal_samples[!, nodes]
...
n_nodes = ground_truth_dag.graph.nodes |> length
...
bn = fit_dag!(deepcopy(bn0), normal_samples)
...
df_outlier_samples = outlier_samples[!, nodes]
...
@info("scorer leaf normal (μ, σ) = $leaf_marginal_scores");
@info("scorer leaf outlier (μ, σ) = $outlier_marginal_scores");
...
ir = rand(1:n_samples, n_reference_samples)
...
function get_ϵ_rankings()
    ...
    for j = 1:size(ϵ, 1)
        ...
        score = zeros(length(nodes))
        for q in 1:max_k
            iq = findfirst(==(Symbol(ranking[q])), nodes)
...
df = DataFrame(n_nodes=Int[], n=Int[], e = Int[],
...
for k=1:min_depth
    ...
    push!(df, @> [n_nodes, n_outlier_nodes, n_outlier_edges,
...
fname = "results/micro-services.csv"
