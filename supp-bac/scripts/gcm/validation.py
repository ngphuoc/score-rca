...
from gcm.cms import InvertibleStructuralCausalModel
from gcm.graph import DirectedGraph, get_ordered_predecessors, is_root_node, validate_causal_graph
...
def refute_invertible_model(
    ...
    :py:class:`InvertibleFunctionalCausalModel <gcm.graph.InvertibleFunctionalCausalModel>` (e.g. the causal mechanisms are
    :py:class:`AdditiveNoiseModels <gcm.AdditiveNoiseModel>` and/or :py:class:`PostNonlinearModels <gcm.PostNonlinearModel>`).
    For this, it is checked if the residual of a causal mechanism is independent of the mechanism's input
    (i.e. we assume causal sufficiency here). For instance, :py:class:`PostNonlinearModels <gcm.PostNonlinearModel>` represent
        Y = f(g(X) + N),
