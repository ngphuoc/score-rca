...
function BayesNet(cpds::AbstractVector{T}; use_topo_sort=true) where {T <: CPD}
    ...
    if use_topo_sort
        (dag, cpds, name_to_index) = _enforce_topological_order(dag, cpds, name_to_index)
    end

	BayesNet(dag, cpds, name_to_index)
end
