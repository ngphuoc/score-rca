...
function MlpUnet(input_dim::Int64, hidden_dims=[100, 50, 20, 10], embed_dim=100, scale=30.0f0, activation=swish)
    I, A, B, C, D, E = (input_dim, hidden_dims..., embed_dim)
    return MlpUnet((
                    embed = Chain(
                                  RandomFourierFeatures(embed_dim, scale),
...
function (unet::MlpUnet)(x::AbstractArray{T,N}, t) where {T,N}
    ...
    #-- Scaling Factor
    σ_t = expand_dims(marginal_prob_std(t), N-1)
    h ./ σ_t
end
